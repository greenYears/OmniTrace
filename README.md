...
- `src/`：React 前端，负责三栏布局、过滤、会话列表和详情展示。
- `src-tauri/src/commands.rs`：Tauri command 入口，暴露 `scan_sources` 与 `get_session_detail`。
...
应用启动后会自动扫描本机真实历史记录。右上角 `扫描` 按钮可以手动重新扫描。
